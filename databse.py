...
class Database_Soumee():
	...
	def write_user_table(self, user_info):
		...
		if len(self.db.execute("SELECT password FROM users WHERE passwords= :password ",{"password":password}).fetchall())== 0 and len(self.db.execute("SELECT email FROM users WHERE email =:email ", {"email":email}).fetchall())== 0:
